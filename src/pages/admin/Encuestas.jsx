...
// ── Modal: solicitar nueva encuesta ──
      ...
      <div className={styles.modalContent}>
        ...
        <form onSubmit={handleSubmit} className={styles.modalBody}>
          <div className={styles.formGroup}>
            ...
            <input value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} placeholder="Ej: Satisfacción con gestión 2025" required />
          </div>
          <div className={styles.formGroup}>
            <label>Descripción / objetivo</label>
            <textarea value={form.descripcion} onChange={e => setForm(f => ({ ...f, descripcion: e.target.value }))} placeholder="¿Qué querés medir con esta encuesta?" rows={3} />
          </div>
      ...
      for (const equipoId of asignados) {
        if (!selected.has(equipoId)) {
          await supabase.from('encuestas_equipo').delete()
            .eq('encuesta_id', encuesta.id).eq('equipo_id', equipoId)
        }
      }
      onSaved(); onClose()
...
// ── Tarjeta de encuesta ──
function EncuestaCard({ encuesta, onApprove, onAssign, onView }) {
  ...
  return (
    ...
    >
      ...
      {encuesta.descripcion && (
        <p className={styles.encuestaDesc}>{encuesta.descripcion}</p>
      )}
      ...
      <div className={styles.encuestaActions} onClick={e => e.stopPropagation()}>
        {paraRevisar && (
          <button onClick={onApprove} style={{
            ...
            border: 'none', borderRadius: 'var(--r)', fontSize: 13, fontWeight: 600,
        ...
        {esPublicada && (
          <button onClick={onAssign} style={{
            padding: '7px 14px', background: 'var(--accent-light)', color: 'var(--accent2)',
            border: '1.5px solid var(--accent2)', borderRadius: 'var(--r)', fontSize: 13,
...
export default function Encuestas() {
  ...
  const [assigning, setAssigning]     = useState(null) // { encuesta, asignados[] }
  ...
  async function fetchData() {
    ...
    const [encRes, eqRes] = await Promise.all([
      supabase.from('encuestas')
        .select('*, encuestas_equipo(equipo_id)')
  ...
  return (
    <div className={styles.page}>
      <Topbar
        title="Encuestas"
        action={{ label: '+ Solicitar encuesta', onClick: () => setShowRequest(true) }}
      />
        ...
        {loading ? <Spinner center size="lg" /> : (
          filtradas.length === 0 ? (
            <div className={styles.empty}>
              <p>{filtro === 'todas'
                ? 'No tenés encuestas todavía.'
                : `No hay encuestas en "${ESTADO_CONFIG[filtro]?.label}".`}
              </p>
              <button
