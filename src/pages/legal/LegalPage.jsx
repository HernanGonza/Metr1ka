...
function Logo({ isDark }) {
  ...
  return (
    <svg width="100" height="9" viewBox="0 0 751 62" fill="none" xmlns="http://www.w3.org/2000/svg"
...
const CONFIGS = {
  privacidad: { icon: '🔒', title: 'Política de Privacidad', updated: 'Abril 2026' },
  terminos:   { icon: '📋', title: 'Términos de Uso',        updated: 'Abril 2026' },
  cookies:    { icon: '🍪', title: 'Política de Cookies',    updated: 'Abril 2026' },
}

export default function LegalPage({ type, content }) {
  const { isDark } = useTheme()
  const cfg = CONFIGS[type] || CONFIGS.privacidad
    ...
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <Link to="/" className={styles.logoLink}><Logo isDark={isDark} /></Link>
        </div>
      </header>

      <div className={styles.hero}>
        <div className={styles.heroInner}>
          <span className={styles.heroIcon}>{cfg.icon}</span>
          <h1 className={styles.heroTitle}>{cfg.title}</h1>
          <p className={styles.heroMeta}>Última actualización: {cfg.updated}</p>
        </div>
      </div>
      ...
      <main className={styles.main}>
        ...
        <div className={styles.related}>
          <p className={styles.relatedTitle}>Otras políticas</p>
          <div className={styles.relatedLinks}>
            {type !== 'privacidad' && <Link to="/privacidad" className={styles.relatedLink}>🔒 Política de Privacidad</Link>}
            {type !== 'terminos'   && <Link to="/terminos"   className={styles.relatedLink}>📋 Términos de Uso</Link>}
            {type !== 'cookies'    && <Link to="/cookies"    className={styles.relatedLink}>🍪 Política de Cookies</Link>}
          </div>
        </div>
      </main>
      ...
      <footer className={styles.footer}>
        <div className={styles.footerInner}>
          <Link to="/" className={styles.backLink}><ArrowLeft size={14} /> Volver al inicio</Link>
          <span className={styles.footerCopy}>© {new Date().getFullYear()} METR1KA · Hecho en Misiones, Argentina 🇦🇷</span>
        </div>
      </footer>
