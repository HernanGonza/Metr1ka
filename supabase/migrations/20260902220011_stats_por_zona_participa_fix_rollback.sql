-- ROLLBACK Migración 11 — restaura get_stats_por_zona de la migración 5.
-- Responde directo: dónde se hicieron (zona por GPS), a quién pertenecen y cuántas por zona y por encuestador.
-- Incluye el bucket zona_id = NULL como "Sin zona" (no se pierde nada).
-- Función nueva: nadie depende de ella. Rollback: drop.
...
begin
  ...
  with sess as (
    select
      ...
      sr.zona_por_gps,
      case
        when v_hay_participa then exists (
